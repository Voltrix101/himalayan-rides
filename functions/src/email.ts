...
export async function sendBookingConfirmationEmail(bookingData: any, paymentData: any): Promise<void> {
  try {
    ...
    const invoiceUrl = await uploadPDFAndGetURL(invoicePath, bookingData.id, 'invoice');
    const tripDetailsUrl = await uploadPDFAndGetURL(tripDetailsPath, bookingData.id, 'trip-details');
...
async function sendEmail(
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      host: functions.config().mail?.host || 'smtp.gmail.com',
      port: parseInt(functions.config().mail?.port || '587'),
      secure: false,
      auth: {
        user: functions.config().mail?.user || 'your-email@gmail.com',
        pass: functions.config().mail?.pass || 'your-app-password',
      },
    });
    ...
    // Recipients
    const adminEmails = functions.config().admin?.emails || 'amritob0327.roy@gmail.com,amritoballavroy@gmail.com';
    const recipients = [bookingData.email, ...adminEmails.split(',')].filter(Boolean);

    // Send email
    const mailOptions = {
      from: `"Himalayan Rides" <${functions.config().mail?.from || functions.config().mail?.user}>`,
      to: recipients.join(','),
      subject,
      text: textContent,
      html: htmlContent,
